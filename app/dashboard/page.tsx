  ...
  return (
    <div className="space-y-7 section-fade">
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: "clamp(1.35rem,4vw,1.75rem)", fontWeight: 700, color: "#fff", lineHeight: 1.1 }}>
            My Subscriptions
          </h1>
          <p style={{ fontSize: 13, color: "rgba(113,113,122,0.7)", marginTop: 6 }}>Manage your active plans and billing</p>
        </div>
        <ClaimUSDC />
      </div>
            ...
            return (
              <div key={sub.planId}
                style={{
                  ...
                  gap: isMobile ? 12 : 16, padding: isMobile ? "16px" : "18px 20px",
                  borderRadius: 16,
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontWeight: 600, fontSize: 14, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{sub.planName} Plan</p>
                    <p style={{ fontSize: 11.5, color: "rgba(113,113,122,0.7)", marginTop: 2 }}>
  ...
  return (
    <div className="space-y-7 section-fade">
      ...
      <div style={{
        ...
        {isLoading ? (
          <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 8 }}><SubRowSkeleton /><SubRowSkeleton /></div>
        ) : history.length === 0 ? (
            ...
            {history.map((row, i) => (
              <div key={row.txHash + i} style={{
                ...
                <span style={{
                  ...
                  borderRadius: 999, fontSize: 11, flexShrink: 0,
                  fontFamily: "ui-monospace,'SF Mono',monospace",
                  ...
                  {row.type === "sub" ? "Sub" : "Renew"}
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, color: "#fff", fontWeight: 500 }}>Plan #{row.planId}</p>
                  <p style={{ fontSize: 11, color: "rgba(113,113,122,0.6)", fontFamily: "monospace", marginTop: 2 }}>#{row.blockNumber}</p>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <p style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", fontFamily: "monospace" }}>{usdcDisplay(row.amount)}</p>
                  <a href={`https://sepolia.voyager.online/tx/${row.txHash}`} target="_blank" rel="noopener noreferrer"
                    style={{ fontSize: 11, color: "rgba(139,92,246,0.7)", textDecoration: "none" }}>
...
function SectionSettings({ address }: { address?: string }) {
  ...
  return (
    <div className="section-fade" style={{ maxWidth: "min(560px, 100%)", display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#fff", lineHeight: 1.1 }}>
          Settings
        </h1>
  ...
  return createPortal(
    <div className="md:hidden">
      ...
      <aside style={{
        ...
        {/* User nav */}
        <p style={{ padding: "12px 14px 6px", fontSize: 10, fontFamily: "ui-monospace,'SF Mono',monospace", color: "rgba(113,113,122,0.5)", letterSpacing: "0.12em", textTransform: "uppercase" }}>User</p>
        {navItems.map(item => {
          const active = section === item.key;
          return (
            <button key={item.key} onClick={() => { setSection(item.key); onClose(); }}
              style={{
                ...
                color: active ? "#fff" : "rgba(113,113,122,0.8)", fontSize: 14,
        ...
        {/* Merchant */}
        <p style={{ padding: "16px 14px 6px", fontSize: 10, fontFamily: "ui-monospace,'SF Mono',monospace", color: "rgba(113,113,122,0.5)", letterSpacing: "0.12em", textTransform: "uppercase", borderTop: "0.5px solid rgba(255,255,255,0.07)", marginTop: 8 }}>Merchant</p>
        <Link href="/merchant" onClick={onClose}
          style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", borderRadius: 10, textDecoration: "none", color: "rgba(113,113,122,0.8)", fontSize: 14, borderLeft: "2px solid transparent" }}>
          <span style={{ color: "rgba(113,113,122,0.6)", flexShrink: 0 }}>{IcoMerchant}</span>
          Revenue Dashboard
        </Link>
        ...
        {address && (
          <div style={{ marginTop: "auto", paddingTop: 16, borderTop: "0.5px solid rgba(255,255,255,0.07)" }}>
            ...
            <button onClick={() => { disconnect(); onClose(); router.push("/"); }}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", borderRadius: 10, background: "none", border: "none", color: "rgba(113,113,122,0.5)", fontSize: 12, cursor: "pointer", marginTop: 4, textAlign: "left", transition: "color 0.15s" }}
              onMouseEnter={e => { e.currentTarget.style.color = "#f87171"; e.currentTarget.style.background = "rgba(239,68,68,0.05)"; }}
              onMouseLeave={e => { e.currentTarget.style.color = "rgba(113,113,122,0.5)"; e.currentTarget.style.background = "none"; }}
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen flex" style={BG}>
      ...
      <main style={{ flex: 1, padding: "clamp(20px,4vw,40px) clamp(16px,4vw,40px)", minWidth: 0 }}>
